...
{
    static string fbxPath = "Arts/Charactor/Weapon";
    static string outPath = "Resources/Arts/Charactor/Weapon";
    [MenuItem("KOL/Actor/Weapon/Gen All")]
    ...
    [MenuItem("KOL/Actor/Weapon/Gen Select")]
    ...
    public static void Split(GameObject fbx)
    {
        foreach (Renderer render in fbx.GetComponentsInChildren<Renderer>(true))
        {
            ...
            if (objInstantiate.transform.parent.parent != null)
            {
                
                GameObject objRoot = objInstantiate.transform.parent.root.gameObject;
                weaponName = objRoot.gameObject.name;
                objInstantiate.transform.parent.parent = null;
                objInstantiate.transform.parent.name = weaponName;
                GameObject.DestroyImmediate(objRoot);
            }
            else
            {
                weaponName = render.gameObject.name;
                objInstantiate.transform.parent.name = render.gameObject.name;
            }
            ...
            PrefabUtility.CreatePrefab(string.Format("{0}/{1}.prefab", prefabPath, renderInstantiate.transform.parent.name), renderInstantiate.transform.parent.gameObject);

            GameObject.DestroyImmediate(objInstantiate.transform.parent.gameObject);
            ...
            if (renderInstantiate is SkinnedMeshRenderer)
            {
                ...
                AssetDatabase.CreateAsset(holder, string.Format("{0}/{1}.asset", bonesPath, weaponName));

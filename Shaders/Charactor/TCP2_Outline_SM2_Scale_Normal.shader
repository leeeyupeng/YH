...
Shader "Hidden/Toony Colors Pro 2/Outline Only Scale Normal (Shader Model 2)"
{
	Properties
	{
		//OUTLINE
		...
		_ScaleLength("ScaleLength",Range(0,0.05)) = 0.01
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma multi_compile TCP2_NONE TCP2_COLORS_AS_NORMALS
			...
			v2f vert (a2v v)
			{
				...
				if (v.color.a != 0 && _Scale != 0)
				{
					...
					v.vertex += -(1 - v.color.a * _Scale) * float4(normalize(v.normal) * _ScaleLength, 0);
				}
